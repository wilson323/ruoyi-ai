...
## 十二、运维取舍备忘（2026-09 治理轮）

以下决策已在代码注释与 `docs/ipd-系统说明/log.md` 披露，此处汇总供部署/DBA 一眼可见：

| 取舍 | 现值 | 得 | 失 | 何时要改 |
|---|---|---|---|---|
| HikariCP `connectionTimeout` | 5000ms（QA05 P1-3 / MED-7） | 高并发下快速失败，避免 30s 排队雪崩 | 慢 SQL/大事务可能被误杀（拿连接超时先于慢查询完成） | 出现大量 `SQLTransientConnectionException` 且慢查询属预期时，调回更大值并优先治理慢 SQL |
| 池 `maxPoolSize` | 基座 20 / dev·prod 80（QA05 P1-1：4 数据源 ×20=80≤151 连接预算） | 满足并发 | 占用 DB 连接 | MySQL `max_connections` 下调时同步重算 |
| JVM 堆（Dockerfile） | `-Xms1g -Xmx2g` 可 env 覆盖（P2-3，2026-09-09） | 避免 ZGC 下默认堆来回伸缩的停顿抖动 | 容器内存 <4g 时需下调 | 大堆需求或容器限额变更时设 `JAVA_XMS`/`JAVA_XMX` |

---

**本文件最后更新**：2026-09-09，Batch-5 增补 §十一 prod 部署参数披露；治理轮 R21 增补 §十二 运维取舍备忘（双线合并轮统一章节号）。
