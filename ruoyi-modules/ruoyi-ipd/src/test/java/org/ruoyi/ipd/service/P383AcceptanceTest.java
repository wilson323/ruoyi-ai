 ...
 *   <li>AC-INC-10：主动退出项目 ⇒ 当月即停发津贴 + 成功奖金资格作废（BR-INC-12；
 *   2026-09-09 owner 拍板「当月退出不发」，原「次月停发」宽松口径作废，
 *   新口径由 P333AcceptanceTest 同步盯守，见 AllowanceService#isMemberActiveInMonth）</li>
...
class P383AcceptanceTest {
    ...
    @Test
    @DisplayName("[AC-INC-10] exitDate 9-15 → 9 月即停（2026-09-09 拍板：当月退出不发）；10/11 月不发")
    void AC_INC_10_退出当月即停() throws Exception {
        Date exitDate = date("2026-09-15");
        assertThat(allowanceService.isMemberActiveInMonth(exitDate, "2026-09")).isFalse();
    ...
    @Test
    @DisplayName("[AC-INC-10] 边界：exitDate 9-1 当月即停；exitDate 10-1 时 9 月整月在岗仍 active")
    void AC_INC_10_月初退出边界() throws Exception {
        Date exitDate = date("2026-09-01");
        assertThat(allowanceService.isMemberActiveInMonth(exitDate, "2026-09")).isFalse();
        assertThat(allowanceService.isMemberActiveInMonth(exitDate, "2026-10")).isFalse();
        Date exitNextMonthStart = date("2026-10-01");
        assertThat(allowanceService.isMemberActiveInMonth(exitNextMonthStart, "2026-09")).isTrue();
    }
    ...
    @Test
    @DisplayName("[AC-INC-10/BR-INC-12] ProjectMember 综合判定：未退出→按 joinDate；已退出→当月即停（拍板口径）")
    void AC_INC_10_综合判定() throws Exception {
        ...
        assertThat(allowanceService.isMemberActiveForMonth(exited, "2026-09")).isFalse();
