...
/**
 * HrApiClient 单测（FA-HR-Sync·2026-09-21 R149，@Tag dev）。
 *
 * <p>本轮目标：仅验证 DTO 解析路径稳定（HR 文档 §3.2.1.8/§3.2.2.8 出参示例）。
 * HttpClient 真实调用 + token 续签重试留 FA-HR-Sync-D1 集成阶段（在 D2 脚本里跑 Mock HR 服务）。
 */
@Tag("dev")
...
class HrApiClientTest {
    ...
    @Test
    @DisplayName("HrResponse.PersonRow 解析：HR §3.2.1.8 出参示例首条")
    void parseUserInfo_docExample() throws Exception {
        // 截取文档示例第一条（PERNR=00000017 大连区域总经理）
        String json = "{ \"code\": 0, \"data\": { \"HEAD\": {}, \"RETURN\": {\"STATUS\":\"S\"},"
            + " \"BODY\": ["
            + " { \"PERNR\":\"00000017\","
            + "   \"BASIC_INFO\": { \"NACHN\":\"陈守一\", \"RUFNM\":\"\", \"GESCH\":\"1\", \"NACHN\":\"陈守一\","
        ...
        List<HrResponse.PersonRow> rows = parseViaReflection(body, HrResponse.PersonRow.class);
        assertThat(rows).hasSize(1);
        HrResponse.PersonRow row = rows.get(0);
        assertThat(row.pernr).isEqualTo("00000017");
        assertThat(row.basicInfo.orgEh).isEqualTo("10000342");
    ...
    @Test
    @DisplayName("HrResponse.OrgRow 解析：HR §3.2.2.8 出参示例首条")
    void parseOrgInfo_docExample() throws Exception {
        ...
        JsonNode root = om.readTree(json);
        JsonNode body = root.path("data").path("BODY");
        List<HrResponse.OrgRow> rows = parseViaReflection(body, HrResponse.OrgRow.class);
        assertThat(rows).hasSize(1);
        ...
        assertThat(row.delFlag).isNull(); // 注意：本例 DELFLAG 为空字符串，未映射到 delFlag
    }
    ...
    // ───── 工具：用反射简化 DTO 解析，绕开 HUTOOL toBean 对嵌套字段的不兼容 ─────

    private <T> List<T> parseViaReflection(JsonNode array, Class<T> klass) {
        // 调用方为单测，本类不受 prod 影响：避免在主代码里写反射，保持主代码清晰。
        // 这里走 cn.hutool.json.JSONUtil.toBean 实现基本一致；如断言失败，检查 camelCase 转换。
