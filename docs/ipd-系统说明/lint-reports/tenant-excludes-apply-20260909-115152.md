# R25 P0-4 租户白名单配置/DDL 对账报告（20260909-115152）
...
| Entity @TableName 数 | 57 |
| DB 实际表数 | 147 |
| 🔴 RC-3-A 配置先行（excludes 已登记但 DB 无表） | 2 |
| 🔴 RC-3-B 实体存在但 DB 未建表（致命） | 0 |
| 🟡 RC-3-C Entity 与 excludes 重叠（需复核） | 56 |
...
| `person_roles` | 从 excludes 移除 OR 补 DDL |
