...
PREPARE stmt FROM @add_file_hash_col;

EXECUTE stmt;

DEALLOCATE PREPARE stmt;

ALTER TABLE knowledge_attach MODIFY COLUMN `doc_id` varchar(32) NULL DEFAULT NULL COMMENT '文档ID';
...
PREPARE stmt FROM @add_file_hash;
...
PREPARE stmt FROM @add_fid_col;

EXECUTE stmt;

DEALLOCATE PREPARE stmt;

ALTER TABLE knowledge_fragment MODIFY COLUMN `doc_id` varchar(32) NULL DEFAULT NULL COMMENT '文档ID';
...
PREPARE stmt FROM @drop_idx_fid;
...
PREPARE stmt FROM @add_tenant_user;
...
PREPARE stmt FROM @add_tenant_share;
