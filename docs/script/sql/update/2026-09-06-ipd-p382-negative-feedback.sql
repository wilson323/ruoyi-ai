...
-- [idem-guard: ALTER negative_feedbacks.IF]
SET @col_exists := (SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='negative_feedbacks' AND COLUMN_NAME='IF');

SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `trigger_type` varchar(32) NOT NULL
    COMMENT ''REWORK_EXCEEDED|QUALITY_ACCIDENT|SPEC_PILE_COPY|MISSED_MARKET_WINDOW（BR-INC-10）''
    AFTER `project_id`',
  'SELECT ''negative_feedbacks.IF exists, skip'' AS msg');
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `main_role` varchar(16) NOT NULL DEFAULT ''MARKET_PM''
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `main_person_id` bigint DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `related_role` varchar(16) DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `related_person_id` bigint DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `main_execution` varchar(32) NOT NULL DEFAULT ''STOP_ALLOWANCE''
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `related_execution` varchar(32) DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `bonus_disqualify` tinyint(1) NOT NULL DEFAULT 0
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `tier_delta` decimal(5',
  'SELECT ''negative_feedbacks.IF exists, skip'' AS msg');
...
ALTER TABLE negative_feedbacks 2) NOT NULL DEFAULT 0
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `trigger_month` varchar(7) DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `recovery_month` varchar(7) DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `triggered_by` bigint DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `decided_by` bigint DEFAULT NULL
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `decided_at` datetime DEFAULT NULL
    COMMENT ''认定时间''
    AFTER `decided_by`',
  'SELECT ''negative_feedbacks.IF exists, skip'' AS msg');
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `lifted_by` bigint DEFAULT NULL
    COMMENT ''解除人''
    AFTER `decided_at`',
  'SELECT ''negative_feedbacks.IF exists, skip'' AS msg');
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `lifted_at` datetime DEFAULT NULL
    COMMENT ''解除时间''
    AFTER `lifted_by`',
  'SELECT ''negative_feedbacks.IF exists, skip'' AS msg');
...
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE negative_feedbacks ADD COLUMN IF NOT EXISTS `decision_comment` varchar(500) DEFAULT NULL
    COMMENT ''认定/解除备注（≤500）''
    AFTER `lifted_at`',
  'SELECT ''negative_feedbacks.IF exists, skip'' AS msg');
