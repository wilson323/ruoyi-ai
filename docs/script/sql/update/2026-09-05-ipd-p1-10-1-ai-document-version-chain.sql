...
-- [idem-guard: ALTER ai_documents.UNIQUE]
SET @col_exists := (SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='ai_documents' AND COLUMN_NAME='UNIQUE');
SET @ddl := IF(@col_exists=0,
  'ALTER TABLE ai_documents
    ADD UNIQUE INDEX uk_ai_doc_parent (parent_version_id)',
  'SELECT ''ai_documents.UNIQUE exists, skip'' AS msg');
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
